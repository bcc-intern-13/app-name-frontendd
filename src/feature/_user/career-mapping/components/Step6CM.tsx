...
import { Step6Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO6_OPTIONS = [
  'Orang yang kreatif dan punya selera estetika tinggi',
  'Orang yang jago teknologi dan problem solver',
  'Orang yang enak diajak ngobrol dan sabar menjelaskan',
  'Orang yang terorganisir, detail, dan tidak suka setengah-setengah',
]

const NO6_OPTIONS_WITH_LABELS = createLabelsOptions(NO6_OPTIONS)

export function Step6CM() {
  ...
  const onSubmit = (formData: Step6Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO6_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Kata orang-orang di sekitarmu, kamu dikenal sebagai?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO6_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
