...
import { Step20Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO20_OPTIONS = [
  'Saya bekerja karena ini mengekspresikan diri dan berkreasi',
  'Saya bekerja karena ingin terus berinovasi dan memecahkan masalah',
  'Saya bekerja karena koneksi dengan orang lain dan ingin memberikan dampak positif',
  'Saya bekerja karena suka keteraturan dan kepuasan terbesar adalah menyelesaikan sesuatu',
]

const NO20_OPTIONS_WITH_LABELS = createLabelsOptions(NO20_OPTIONS)

export function Step20CM() {
  ...
  const onSubmit = (formData: Step20Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO20_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Kalimat mana yang paling menggambarkan motivasi kerjamu?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO20_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
