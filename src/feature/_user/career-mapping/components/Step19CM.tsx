...
import { Step19Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO19_OPTIONS = [
  'Buat visualisasi masalah agar semua bisa melihat gambaran besarnya',
  'Menganalisis akar masalahnya secara objektif berdasarkan data',
  'Jadi mediator atau ajak semua refleksi dan belajar dari situasi ini',
  'Mendokumentasikan masalah, membuat SOP, atau fokus ke solusi praktis yang bisa dieksekusi',
]

const NO19_OPTIONS_WITH_LABELS = createLabelsOptions(NO19_OPTIONS)

export function Step19CM() {
  ...
  const onSubmit = (formData: Step19Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO19_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Kalau ada konflik dalam tim, kamu cenderung?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO19_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
