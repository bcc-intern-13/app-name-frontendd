...
import { Step12Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO12_OPTIONS = [
  'Diminta kerja tanpa ada ruang untuk berekspresi',
  'Harus kerja tanpa data atau logika yang jelas',
  'Tidak ada komunikasi dan harus kerja sendiri terus',
  'Sistem berantakan atau pekerjaan tidak selesai dan hasilnya tidak terukur',
]

const NO12_OPTIONS_WITH_LABELS = createLabelsOptions(NO12_OPTIONS)

export function Step12CM() {
  ...
  const onSubmit = (formData: Step12Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO12_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Kamu merasa paling tidak nyaman saat?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO12_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
