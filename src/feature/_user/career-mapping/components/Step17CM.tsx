...
import { Step17Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO17_OPTIONS = [
  'Karya yang diakui dan diapresiasi banyak orang',
  'Sistem atau solusi yang kamu bangun benar-benar berfungsi',
  'Seseorang bilang kamu sangat membantu atau mengubah cara pandang mereka',
  'Proyek selesai tepat waktu, rapi, atau target tercapai dan hasilnya langsung dirasakan',
]

const NO17_OPTIONS_WITH_LABELS = createLabelsOptions(NO17_OPTIONS)

export function Step17CM() {
  ...
  const onSubmit = (formData: Step17Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO17_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Tipe pencapaian mana yang paling membuatmu bangga?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO17_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
