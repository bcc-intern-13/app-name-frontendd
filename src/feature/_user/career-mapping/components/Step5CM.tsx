...
import { Step5Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO5_OPTIONS = [
  'Yang bikin materi presentasi atau desain visualnya',
  'Yang urus tools, sistem, atau infrastruktur digitalnya',
  'Yang jadi penghubung antaranggota tim',
  'Yang kelola jadwal dan dokumentasi atau pastikan semua tugas selesai tepat waktu',
]

const NO5_OPTIONS_WITH_LABELS = createLabelsOptions(NO5_OPTIONS)

export function Step5CM() {
  ...
  const onSubmit = (formData: Step5Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO5_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Di tim baru, peran apa yang paling natural kamu ambil?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO5_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
