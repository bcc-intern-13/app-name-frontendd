...
import { Step7Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO7_OPTIONS = [
  'Coba pendekatan visual atau kreatif yang berbeda',
  'Analisis di mana sistem atau logikanya yang salah',
  'Cari feedback dari orang lain',
  'Review ulang catatan atau ulangi dari awal dengan lebih hati-hati',
]

const NO7_OPTIONS_WITH_LABELS = createLabelsOptions(NO7_OPTIONS)

export function Step7CM() {
  ...
  const onSubmit = (formData: Step7Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO7_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Kalau kamu gagal dalam sesuatu, reaksi pertamamu?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO7_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
