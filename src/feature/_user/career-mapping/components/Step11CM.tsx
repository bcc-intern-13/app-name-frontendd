...
import { Step11Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO11_OPTIONS = [
  'Langsung praktik dan lihat hasilnya secara visual',
  'Pahami konsep dasarnya dulu baru eksekusi',
  'Belajar bareng orang lain atau join komunitas',
  'Ikut panduan step by step atau langsung kerjakan task nyata',
]

const NO11_OPTIONS_WITH_LABELS = createLabelsOptions(NO11_OPTIONS)

export function Step11CM() {
  ...
  const onSubmit = (formData: Step11Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO11_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Saat kamu harus belajar skill baru, kamu lebih suka?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO11_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
