...
import { Step14Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO14_OPTIONS = [
  'Screenshot atau simpan referensi visual yang menarik',
  'Cari tahu cara kerja teknologi atau aplikasi baru',
  'Menghubungi teman lama atau mencatat hal menarik yang baru dipelajari',
  'Membuat list atau kategorisasi atau menyelesaikan tugas kecil yang tertunda',
]

const NO14_OPTIONS_WITH_LABELS = createLabelsOptions(NO14_OPTIONS)

export function Step14CM() {
  ...
  const onSubmit = (formData: Step14Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO14_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Apa yang paling sering kamu lakukan secara spontan?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO14_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
