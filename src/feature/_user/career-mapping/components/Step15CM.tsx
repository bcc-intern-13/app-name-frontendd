...
import { Step15Data, stepSchema, type Step1Data } from '@/lib/validations/careerMapping'
...
const NO15_OPTIONS = [
  ...
  'Sistematis / Eksekutif',
]

const NO15_OPTIONS_WITH_LABELS = createLabelsOptions(NO15_OPTIONS)

export function Step15CM() {
  ...
  const onSubmit = (formData: Step15Data) => {
    const labelOnly = getLabelFromOptions(formData.answer, NO15_OPTIONS_WITH_LABELS)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <h2 className="h2-bold text-[#252525] mb-2">
          Kalau diminta memilih satu kata untuk menggambarkan cara kerjamu?
        </h2>
      </div>

      <FormField error={errors.answer?.message}>
        <div className="grid gap-3">
          {NO15_OPTIONS_WITH_LABELS.map((option) => (
            <button
              key={option.label}
              type="button"
              onClick={() =>
